...
def upload_data_to_staging(filename, date, pg_table, pg_schema, ti):
    ...
    query = f'''DELETE FROM {pg_schema}.{pg_table} WHERE date_time::date = '{date}'::date
            '''
...
with DAG(
        ...
        schedule_interval='5 6 * * 1', # Запускаем раз в неделю, т.к. срез по отчету недельный
) as dag:
